...
Codex profiles, toolset intent, model routing placeholders, worktree
...
- `profiles.yml`: four Codex profiles and their skill/tool boundaries.
...
- `scripts/codex/dry-run.sh`: credential-free local gate runner. It writes
  reports to `${TMPDIR:-/tmp}/kansobooks-codex-runs` by default; set
  `CODEX_REPORT_ROOT=docs/content-runs` only when a report should be
  intentionally versioned.
- `docs/content-runs/_templates/`: run-log and exception report templates.

Requires local Mac Mini setup:

- Codex installation and scheduler choice.
- Local provider credentials in macOS Keychain or the Codex-supported secret store.
